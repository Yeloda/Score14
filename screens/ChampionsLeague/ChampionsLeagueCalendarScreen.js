...
const ChampionsLeagueCalendarScreen = ({navigation, route}) => {
    ...
    const fetchDateMatches = async (date) => {
        ...
        fetch("https://v3.football.api-sports.io/fixtures?season=2024&league=2&date="+searchDate, {"method": "GET","headers": {"x-rapidapi-host": "v3.football.api-sports.io","x-rapidapi-key": process.env.API_KEY}})
        .then(response => response.json())
        .then(data => {
            data.response.sort((a, b) => a.fixture.date > b.fixture.date ? 1 : -1)
            setListMatchesChampion([...data.response])
...
export default ChampionsLeagueCalendarScreen
